...
if __name__ == '__main__':
    ...
    for env in envs:
        for model_name in ["gru_8_10_12", "wfart_8_10_12"]:
            for run in ["run1", "run2"]:
                mean_reward_per_episode = load_json(os.path.join(
                    'final_experiment', model_name, run, 'mean_reward_per_episode.json'))
                ...
                plt.plot(modified_timestamps, avg_reward,
                         label=f"${model_names[model_name]}$ - Run {run[-1]}")
        ...
        plt.legend()
        plt.savefig(f'custom_graphs/graphs3/{env_names[env]}.png')
        plt.clf()
