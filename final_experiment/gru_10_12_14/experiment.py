...
ray.tune.registry.register_env(
    "LabyrinthEscapeMedium", lambda env_config: labyrinth_escape.LabyrinthEscape(maze_dims=(14, 14)))
ray.tune.registry.register_env(
...
ray.tune.registry.register_env(
    "LabyrinthExploreMedium", lambda env_config: labyrinth_explore.LabyrinthExplore(maze_dims=(14, 14)))
...
for cycle_count in range(num_of_cycles):
    for env in envs:
        ...
        with open(f"{sys.argv[1]}/timesteps.txt", "a") as f:
            f.write(
                f"Timesteps completed in cycle {cycle_count} for environment {env} = {curr_env_timesteps}\n")
